...
f_combineData <- function(exp_names, sim_end_date,trace_selection){
  ...
  dat <- dat_list %>% bind_rows() %>% filter(date <= sim_end_date) %>% as.data.table()
  rm(dat_list)
  return(dat)
}
...
dat %>%
  ...
  dplyr::mutate(peak_before_2021 = ifelse(date<=sim_end_date,1,0)) %>%
...
dat <- dat %>% mutate(peak_beforeDec = ifelse(date_peak <= sim_end_date,1,0))
...
p3A <- ggplot(data=subset(dat,date >=first_plot_date & date <=sim_end_date )) +
  ...
  geom_line(data=subset(dat,date >=first_plot_date & scen_num == unique(dat$scen_num)[1]), aes(x=date, y=516),linetype='dashed',col=capacitycolor,alpha=1,size=1.2) +
  ...
  geom_vline(xintercept=c(baseline_date, last_plot_date))+
  scale_x_date(date_breaks = "1 month", date_labels = "%b") +
  theme_minimal()+customTheme+
  labs(x="",y="ICU occupancy",color="")+
  theme(legend.position = "none")

p3B_dat <- subset(dat,date >=first_plot_date & date <=sim_end_date) %>%
...
p3C_dat <- subset(dat,date >=first_plot_date & date <=sim_end_date) %>%
...
p3BC <- plot_grid(p3B,p3C, ncol=2, align="h", labels=c("B","C"))
pplot <- plot_grid(p3A,p3BC, ncol=1,rel_heights=c(1,0.7), labels=c("A",""))

f_save_plot(
  plot_name = paste0("Fig3_full"), pplot = pplot,
  plot_dir = file.path(fig_dir), width =12, height = 8
)

## Save csvs
fwrite(p3B_dat, file.path(fig_dir,"csv","p3B_full_dat.csv"))
fwrite(p3C_dat, file.path(fig_dir,"csv","p3C_full_dat.csv"))
