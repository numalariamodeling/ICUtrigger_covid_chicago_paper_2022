...
f_export_sumary_csv <- function(use_values_list, fittingParam, npairs = 10, writeYamlSnippet = FALSE) {
  ...
  if (writeYamlSnippet) {
    ...
    yaml_snippet <- suppressWarnings(readLines(file.path(git_dir, "../simulations/experiment_configs", "snippets", "templates", "config_weekly_ki_multiplier_fit.txt")))
    ...
    writeLines(yaml_snippet, file.path(git_dir, "../simulations/experiment_configs", "snippets", "config_weekly_ki_multiplier_fit.txt"))
  }
}
