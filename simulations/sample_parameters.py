...
if __name__ == '__main__':
    ...
    master_config = "extendedcobey_200428.yaml"
    exp_config = "extendedcobey_200428.yaml"
    region = "EMS_11"
    emodl_name = "8_reg11_emodl_triggeredMitigation_percred40.emodl"
    sub_samples = 1000
    csv_name_load= 'sampled_parameters_for_trigger.csv'
    csv_name_combo = 'capacity_multiplier.csv'

    _, _, sim_dir, exe_dir = load_paths(Location='Local')
    Location = "Local"

    emodl_dir = os.path.join(sim_dir, 'emodl')
    cfg_dir = os.path.join(sim_dir, 'cfg')
    yaml_dir = os.path.join(sim_dir, 'experiment_configs')

    dfparam1 = get_parameters(from_configs=False, sample_csv_name=csv_name_load)
    dfparam1 = dfparam1[dfparam1['scen_num']<=400]
    dfparam2 = pd.read_csv(os.path.join('experiment_configs', 'input_csv', csv_name_combo))

    dfparam_raw = gen_combos(csv_base=dfparam1, csv_add=dfparam2)
    dfparam_raw['trigger_EMS_11'] = 516
    dfparam1['trigger_EMS_11'] = 516

    ## Counterfactual
    csv_name_save= 'sampled_parameters_100perc.csv'
    dfparam = dfparam1
    dfparam['trigger_delay_days'] =1
    dfparam['reopening_multiplier_4_EMS_11'] =0.12
    dfparam['capacity_multiplier'] =0
    check_and_save_parameters(df=dfparam, emodl_template=emodl_name, sample_csv_name=csv_name_save)

    csv_name_save= 'sampled_parameters_50perc.csv'
    dfparam = dfparam1
    dfparam['trigger_delay_days'] =1
    dfparam['reopening_multiplier_4_EMS_11'] =0.068
    dfparam['capacity_multiplier'] =0
    check_and_save_parameters(df=dfparam, emodl_template=emodl_name, sample_csv_name=csv_name_save)


    ### Delay 1
    csv_name_save= 'sampled_parameters_100perc_trigger_delay1.csv'
    dfparam = dfparam_raw
    dfparam['trigger_delay_days'] =1
    dfparam['reopening_multiplier_4_EMS_11'] =0.12
    check_and_save_parameters(df=dfparam, emodl_template=emodl_name, sample_csv_name=csv_name_save)

    csv_name_save= 'sampled_parameters_50perc_trigger_delay1.csv'
    dfparam = dfparam_raw
    dfparam['trigger_delay_days'] =1
    dfparam['reopening_multiplier_4_EMS_11'] =0.068
    check_and_save_parameters(df=dfparam, emodl_template=emodl_name, sample_csv_name=csv_name_save)
    ...
    ### Delay 7
    csv_name_save = 'sampled_parameters_100perc_trigger_delay7.csv'
    dfparam = dfparam_raw
    dfparam['trigger_delay_days'] = 7
    dfparam['reopening_multiplier_4_EMS_11'] = 0.12
    check_and_save_parameters(df=dfparam, emodl_template=emodl_name, sample_csv_name=csv_name_save)

    csv_name_save = 'sampled_parameters_50perc_trigger_delay7.csv'
    dfparam = dfparam_raw
    dfparam['trigger_delay_days'] =7
    dfparam['reopening_multiplier_4_EMS_11'] = 0.068
    check_and_save_parameters(df=dfparam, emodl_template=emodl_name, sample_csv_name=csv_name_save)
