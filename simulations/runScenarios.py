...
from setup.simulation_helpers import (DateToTimestep, write_emodl,
...
if __name__ == '__main__':
    ...
    _, wdir, sim_dir, exe_dir = load_paths(Location=args.running_location)
    ...
    log.debug(f"Working directory: sim_dir={sim_dir}")
    log.debug(f"sim_dir={sim_dir}")
    ...
    exp_name = f"{today.strftime('%Y%m%d')}_{args.name_suffix}"

    # Generate folders and copy required files
    temp_dir, temp_exp_dir, trajectories_dir, sim_output_path, plot_path = makeExperimentFolder(
        exp_name, emodl_dir, emodl_template, cfg_dir, args.cfg_template,  yaml_dir,
        args.masterconfig, args.experiment_config, args.intervention_config,wdir=wdir, sim_dir=sim_dir)
    ...
    if Location == 'NUCLUSTER':
        generateSubmissionFile_quest(nscen, exp_name, args.experiment_config, trajectories_dir,sim_dir, temp_exp_dir,exe_dir,sim_output_path,model)
    ...
    if Location == 'Local':
        ...
        try:
            runSamplePlot(exp_name,sim_output_path=sim_output_path, plot_path=plot_path,channel_list_name="custom")
